...
summary: "用 useCallback 稳定函数引用，与 memo 搭配食用最佳"
---
...
这里就不展开讲防抖了。我比较疑惑的点是，`useCallback` 为什么在这里出现，它有什么用？
...
**问题**：每次 `MyComponent` 渲染，`handleClick` 是新函数，`ChildComponent` 会重新渲染，即使它被 `memo` 包裹。
...
const SearchComponent = () => {
  ...
  return <TextInput value={query} onChangeText={handleSearch} placeholder="Type to search" />;
};
```
